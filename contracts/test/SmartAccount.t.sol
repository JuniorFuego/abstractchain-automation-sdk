...
contract SmartAccountTest is Test {
    ...
    uint256 public ownerPrivateKey = 0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef;
    ...
    function testValidateUserOpInvalidSignature() public {
        UserOperation memory userOp = UserOperation({
            ...
            maxPriorityFeePerGas: 1e9,
            paymasterAndData: "",
            signature: abi.encodePacked(bytes32(uint256(1)), bytes32(uint256(2)), uint8(27)) // Invalid signature
        });
    ...
    function testFuzzExecute(address target_, uint256 value_, bytes calldata data_) public {
        ...
        // Avoid precompiled contracts (addresses 1-9) as they may have unexpected behavior
        vm.assume(target_ > address(0x9));
    ...
    function testFuzzValidateUserOp(uint256 nonce_, bytes32 hash_) public {
        // Get current nonce before the test
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerPrivateKey, hash_);
        userOp.signature = abi.encodePacked(r, s, v);
        
        vm.prank(address(entryPoint));
        uint256 validationData = account.validateUserOp(userOp, hash_, 0);
        
        if (nonce_ == currentNonce) {
